...
import "@/app/styles/InfiniteSpiral.css";
...
const InfiniteSpiral = forwardRef(function InfiniteSpiral(
  {
    items = [],
    speed = 0.55,
    direction = "up",
  ...
  useImperativeHandle(ref, () => ({
    setProgress: (p) => {
      isExternalDrivenRef.current = true;
      targetProgressRef.current = p * normalizedItems.length * scrollMultiplier;
    },
  ...
  useEffect(() => {
    ...
    const scrollEnabled =
      (animationMode === "scroll" || animationMode === "all") &&
      !isExternalDrivenRef.current;
    ...
    const handleScroll = () => {
      if (!scrollEnabled || isExternalDrivenRef.current) return;
      const nextScrollY = window.scrollY;
      const scrollDelta = nextScrollY - lastScrollY;
      lastScrollY = nextScrollY;
      if (!visibleRef.current || scrollDelta === 0) return;
    ...
    const render = (time) => {
      ...
      const autoEnabled =
        (animationMode === "auto" || animationMode === "all") &&
        !isExternalDrivenRef.current;
      const motionPaused =
      ...
      if (!isExternalDrivenRef.current) {
        targetProgressRef.current += autoSpeedRef.current * delta;
      }

      const followBlend =
        1 - Math.exp(-delta * (draggingRef.current ? 22 : 14));
      progressRef.current +=
      ...
      // Track currently centered / active item
      ...
      cardRefs.current.forEach((card, index) => {
        ...
        card.style.filter =
          blur > 0.01 ? `blur(${blur.toFixed(2)}px)` : "none";
